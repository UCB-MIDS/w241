...
Look, the benefit to R is the maniuplation of structured data. If you've got to generate a bunch of data through a scraping enterprise, or some form of a complex pipeline, or... you're probably better off handling that at a different layer; either on the storage layer before it comes to the anlaysis layer, or in a python hanlding layer. 

But, once you've got data that's even *close* to being rectangular, I think you're hardpresed to find a more natural way of manipulating than R. pandas comes close, but there's a lot of fiddling around with indexes that can get in the way of intuitive merges and joins. 
...
In what remains, I'll include a little write-up about data.table, which is probably less approachable out of the box, but does scale into medium size data (e.g. 10GB in memory) more readily than dplyr. This is *much less* comprehensive than the great guides that are available from the package authors: 
...
Updating, creating, modifying is performred by reference in data.table, which is a useful construct when we start have medium-sized data (1e6+ rows). Basically, base R is inefficient in its memory management; at times it will allocate 4x the memory that is actually being used in data that you want. Its frustrating.
...
Note here that I use a second idiom of data.tables, the `.N` which just evaluates how many rows are in the particular set you've got. I could have hardcoded that: `dt[id < 10, age.low := sample(18:30, size = 9, replace = T)]` but then if my data changed, I would be left holding the bag...
...
object whose class is a data.table. Meaning that we can keep this as a data.table for futher work, or storage. 
